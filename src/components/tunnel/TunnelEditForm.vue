<template>
  <div class="tunnel-edit-form">
    <n-card title="编辑隧道配置" size="large">
      <template #header-extra>
        <n-tag type="info" size="small">
          节点: {{ tunnelData?.node?.name }}
        </n-tag>
      </template>
...
// 获取预览URL
const getPreviewUrl = () => {
  if (!formData.value.subdomain || !props.tunnelData?.node) return ''
  
  const protocol = formData.value.type === 'https' ? 'https' : 'http'
  const domain = formData.value.custom_domain || props.tunnelData.node.server_addr
...
const saveTunnel = async () => {
  try {
    ...
    await ApiClient.put(`/tunnels/${props.tunnelData.id}`, formData.value)
    message.success('隧道修改成功')
    emit('success')
  } catch (error: any) {
    console.error('保存隧道失败:', error)
    message.error(error.message || '保存隧道失败')
  } finally {
    saving.value = false
  }
}
...
const initFormData = () => {
  if (props.tunnelData) {
    Object.assign(formData.value, {
      name: props.tunnelData.name || '',
      type: props.tunnelData.type || 'http',
      local_ip: props.tunnelData.local_ip || '127.0.0.1',
      local_port: props.tunnelData.local_port || null,
      remote_port: props.tunnelData.remote_port || null,
      custom_domain: props.tunnelData.custom_domain || '',
      subdomain: props.tunnelData.subdomain || '',
      locations: props.tunnelData.locations || ['/'],
      host_header_rewrite: props.tunnelData.host_header_rewrite || '',
      use_encryption: props.tunnelData.use_encryption ?? true,
      use_compression: props.tunnelData.use_compression ?? true,
      bandwidth_limit_upload: props.tunnelData.bandwidth_limit_upload || null,
      bandwidth_limit_download: props.tunnelData.bandwidth_limit_download || null,
      remark: props.tunnelData.remark || ''
    })
  }
}
